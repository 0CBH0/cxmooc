...
function MoocPlayer(option) {
	...
	this.playerPath = option.playerPath || '/ananas/modules/video/cxplayer/player_4.0.8.swf?v=20150402';
...
MoocPlayer.prototype._init = function() {
	...
	url = (function() {
		...
		// 测试用
		if (18068657 == me.data.userid || 269 == me.data.fid) {
			if (me.data.httpmd) {
		...
		if (me.data.http) {
			urls.push("{'公网1标清':'" + me.data.http + "', 'indexorder':0}");
			urls.push("{'公网2标清':'" + me.data.http.replace(/s1.ananas.chaoxing.com/, 's2.ananas.chaoxing.com') + "', 'indexorder':0}");
		}
		
	    if (me.data.httpmd) {
			urls.push("{'公网1极速':'" + me.data.httpmd + "', 'indexorder':0}");
			urls.push("{'公网2极速':'" + me.data.httpmd.replace(/s1.ananas.chaoxing.com/, 's2.ananas.chaoxing.com') + "', 'indexorder':0}");
		}
		
		if (me.data.httphd) {
			urls.push("{'公网1720p':'" + me.data.httphd + "', 'indexorder':0}");
			urls.push("{'公网2720p':'" + me.data.httphd.replace(/s1.ananas.chaoxing.com/, 's2.ananas.chaoxing.com') + "', 'indexorder':0}");
		}
		
		if (me.data.httpshd) {
			urls.push("{'公网11080p':'" + me.data.httpshd + "', 'indexorder':0}");
			urls.push("{'公网21080p':'" + me.data.httpshd.replace(/s1.ananas.chaoxing.com/, 's2.ananas.chaoxing.com') + "', 'indexorder':0}");
		}
		
		if (me.data.cdn && me.data.cdn.length > 0) {
			for (var i = 0,len = me.data.cdn.length; i < len; i++) {
				if (me.data.http) {
					urls.push("{'" + me.data.cdn[i].label + "标清':'" + me.data.cdn[i].url + "/" + removeStart(me.data.http, 'http://') + "', 'indexorder':" +  me.data.cdn[i].indexorder + "}");
				}
				
				if (me.data.httpmd) {
					urls.push("{'" + me.data.cdn[i].label + "极速':'" + me.data.cdn[i].url + "/" + removeStart(me.data.httpmd, 'http://') + "', 'indexorder':" +  me.data.cdn[i].indexorder + "}");
				}
				
				if (me.data.httphd) {
					urls.push("{'" + me.data.cdn[i].label + "720p':'" + me.data.cdn[i].url + "/" + removeStart(me.data.httphd, 'http://') + "', 'indexorder':" +  me.data.cdn[i].indexorder + "}");
				}
				
				if (me.data.httpshd) {
					urls.push("{'" + me.data.cdn[i].label + "1080p':'" + me.data.cdn[i].url + "/" + removeStart(me.data.httpshd, 'http://') + "', 'indexorder':" +  me.data.cdn[i].indexorder + "}");
				}
			}
		}
	...
	me.switchWindow(me.data.enableSwitchWindow == 1);
};
